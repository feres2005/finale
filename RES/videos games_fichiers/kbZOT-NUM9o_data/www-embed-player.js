...
window.console.log(c.join("\n"),e));e=a.stacktrace;c=a.columnNumber;if(a.hasOwnProperty("params"))if(d=a.params,"object"==typeof a.params){var g=0;for(l in d){var h="params."+l,k=String(JSON.stringify(d[l])).substr(0,500);f[h]=k;g+=h.length+k.length;if(500<g)break}}else f.params=String(JSON.stringify(d)).substr(0,500);a=Sb(a);e=e||a.stack;var l=a.lineNumber.toString();isNaN(l)||isNaN(c)||(l=l+":"+c);window.yterr&&Aa(window.yterr)&&window.yterr(a);if(!(Ve.has(a.message)||0<=e.indexOf("/YouTubeCenter.js")||
0<=e.indexOf("/mytube.js"))){l={Xa:{a:"logerror",t:"jserror",type:a.name,msg:a.message.substr(0,250),line:l,level:b,"client.name":f.name},C:{url:O("PAGE_NAME",window.location.href),file:a.fileName},method:"POST"};f.version&&(l["client.version"]=f.version);e&&(l.C.stack=e);for(var n in f)l.C["client."+n]=f[n];if(n=O("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS",void 0))for(var p in n)l.C[p]=n[p];Ie(O("ECATCHER_REPORT_HOST","")+"/error_204",l);Ve.add(a.message);We++}}}
...
{};f.context.user.credentialTransferTokens=[{token:l,scope:h}]}delete og[p];f.requestTimeMs=e;Q("unsplit_gel_payloads_in_logs")&&(f.unsplitGelPayloadsInLogs=!0);if(g=de("EVENT_ID"))h=(O("BATCH_CLIENT_COUNTER",void 0)||0)+1,h>eg&&(h=1),N("BATCH_CLIENT_COUNTER",h),g={serializedEventId:g,clientCounter:h},f.serializedClientEventId=g,fg&&gg&&Q("log_gel_rtt_web")&&(f.previousBatchInfo={serializedClientEventId:fg,roundtripMs:gg}),fg=g,gg=0;ug(b,a,f,{retry:jg.has(a),onSuccess:A(vg,this,U())})}if(d.previousDispatchMs){c=
